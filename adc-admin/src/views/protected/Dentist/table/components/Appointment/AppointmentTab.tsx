...
setErrorAlertOpen(false);
  ...
  const {data, isFetching } = useGet({
    endpoint: 'appointment/getAllUpcoming', 
    param: {id : Id},
  ...
  return (
    <>
      <Paper sx={{ width: "100%", mb: 2, backgroundColor: theme.palette.background.paper }}>
        {isFetching ? (
          <Loader />
        ) : (
          <>
            <TableContainer component={Paper} sx={{ margin: 1 }}>
              <Table sx={{ minWidth: 800 }} aria-label="simple table" size="small">
                <TableHead>
                  <TableRow>
                    <TableCell sx={{backgroundColor: theme.palette.mode === 'dark' ? '#2E2E2E' : theme.palette.grey[300], color: theme.palette.text.primary}} align="left">Patient Name</TableCell>
                ...
                <TableBody>
                  {visibleDataRows.length > 0 ? (
                    visibleDataRows.map((row: Item, index: number) => (
                      ...
                      >
                        <TableCell sx={{ backgroundColor: theme.palette.mode === 'dark' ? (index % 2 === 0 ? '#4E4E4E' : '#616161') : 'default', color: theme.palette.text.primary}} align="left">{row.patient_name}</TableCell>
