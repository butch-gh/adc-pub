...
interface Props {
  updateDentistID : (Id:number)=>void;
}

const DataTable = (props:Props) => {
    let MODULE = 'ADC-ADMIN:Dentist';
    const {updateDentistID} = props;
  ...
  const { logActivity} = useLogActivity();
    ...
    const handleSubmitItem = async () => {
      ...
      //console.log('schedtimedata', schedtimedata)

      const times = Object.fromEntries(
        Object.entries(schedtimedata).filter(([_, value]) => value.from != undefined          
        )
      );
      ...
      const apiParam = {
        ...
        //timeSched: JSON.stringify(getValues('basicInfo.timeSched')),
        schedule: JSON.stringify(getValues('scheduleTimes.schedule')),
        times: JSON.stringify(times)
      };
      //console.log('TRACE TIMESCHED', apiParam)
        if (currentItem) {
          // Update existing item
          const endpoint = '/dentist/update'
      ...
      const handleConfirmDelete = async () => {
        let endpoint = '/dentist/delete'
        let deleteId = getValues('internal.deleteId');        
        try {          
          const response = await apiCall(`${import.meta.env.VITE_API_URL}/appointment/dentist/delete`, {id : deleteId},
          (err) => {setErrorAlertMessage( err.response?.data?.message || 'Something went wrong.'); setErrorAlertOpen(true);});
  ...
  return (
    <> 
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>        
            <Button variant="contained" startIcon={<AddIcon/>} onClick={() => openModal('Add New dentist')}>Add dentist</Button>
