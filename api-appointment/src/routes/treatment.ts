...
router.post('/get', extractUser, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.body;
    const result = await pool.query('SELECT * FROM sf_get_treatment($1);', [id]);
...
router.post('/add', extractUser, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM sf_insert_treatment($1, $2);', [description, duration]);
    const data = (result.rows as Array<{ sf_insert_treatment: any }>).map(row => row.sf_insert_treatment);
...
router.post('/update', extractUser, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM sf_update_treatment($1, $2, $3);', [id, description, duration]);
...
router.post('/delete', extractUser, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.body;
    const result = await pool.query('SELECT * FROM sf_delete_treatment($1);', [id]);
