...
# to start fresh, run: .\start-all-dev-appointment.ps1
...
$wtCommand = 'new-tab --title "Appointment Frontend" cmd /k "cd /d ' + $base + '\adc-admin && npm run dev" ; ' +
             ...
             'move-focus right ; ' +
             'split-pane -H --title "Appointment API" cmd /k "cd /d ' + $base + '\api-appointment && npm run dev"'
...
Write-Host '📐 Layout: 2x2 grid (Frontend + Portal) / (Gateway + Appointment)' -ForegroundColor Cyan