...
# # Frontend
# Start-Process wt -ArgumentList "-w 0 new-tab --title `"Frontend`" cmd /k `"cd /d c:\_kaye\adc-phase2\adc-billing && npm run dev`""
...
# # Billing API
# Start-Process wt -ArgumentList "-w 0 new-tab --title `"Billing API`" cmd /k `"cd /d c:\_kaye\adc-phase2\api-billing && npm run dev`""
...
# to start fresh, run: .\start-all-dev-billing.ps1
...
$wtCommand = 'new-tab --title "Frontend" cmd /k "cd /d ' + $base + '\adc-billing && npm run dev" ; ' +
             ...
             'move-focus right ; ' +
             'split-pane -H --title "Billing API" cmd /k "cd /d ' + $base + '\api-billing && npm run dev"'
